...
def evaluate_ride_full_day2(full_report: List[str], rider: Dict[str, Any]) -> str:
    ...
    prompt = (
        ...
        f'  "fun_fact": "<Motorcycle fact or tip or riding tip, or a motorcycle quote>"\n'
        f'}}'
    )
...
def get_weather_recommendation_summary(should_ride: bool, conditions: Dict) -> str:
    """Generate a brief recommendation summary for email subjects"""
    if should_ride:
        return "🏍️ RIDE - Conditions favorable"
    else:
        main_concern = "weather concerns"
        if conditions.get('evening_rain', 0) > 30:
            main_concern = "evening rain forecast"
        elif conditions.get('morning_rain', 0) > 50:
            main_concern = "heavy rain forecast"
        elif conditions.get('max_wind', 0) > 25:
            main_concern = "high wind conditions"

        return f"🚗 DON'T RIDE - {main_concern}"


def extract_conditions_from_reports(reports: List[str]) -> Dict:
    """Extract key weather conditions from reports for decision summary"""
    conditions = {
        'morning_rain': 0,
        'evening_rain': 0,
        'max_wind': 0,
    ...
    for report in reports:
        rain_values = [int(x) for x in re.findall(r'(\d+)% rain', report)]
        wind_values = [float(x) for x in re.findall(r'([\d.]+) mph', report)]

        if 'Morning' in report and rain_values:
            conditions['morning_rain'] = max(rain_values)
        elif 'Evening' in report and rain_values:
            conditions['evening_rain'] = max(rain_values)

        if wind_values:
            conditions['max_wind'] = max(
                conditions['max_wind'], max(wind_values))
