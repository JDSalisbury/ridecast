...
def parse_user_data(file_path: str = "users.json") -> List[Dict[str, Any]]:
    ...
    for user in raw_data["users"]:
        ...
        ride_in_hours = tuple(
            map(int, user["RIDE_IN_HOURS"].strip("()").split(",")))
        ride_back_hours = tuple(
            map(int, user["RIDE_BACK_HOURS"].strip("()").split(",")))
...
def print_summary(user: Dict[str, Any], forecasts: List[Tuple[str, ForecastResult]], label: str) -> str:
    ...
    all_sources = {f.source for f in [
        OpenWeather(), WeatherAPI(), TomorrowIO(), NOAA()]}
    ...
    if forecasts:
        ...
        for loc_name, result in forecasts:
            ...
            forecast_time = result.forecast_datetime.strftime(
                "%b %d, %I:%M %p")
    ...
    if successful_sources:
        lines.append(
            f"✅ APIs successful: {', '.join(sorted(successful_sources))}")
    if failed_sources:
    ...
    for user in users:
        ...
        try:
            ...
            summary = eval_data.get(
                "summary", "Weather forecast data available")
            ...
            html_body = create_ride_email_html(
                summary, fun_fact, user_first_name)

            send_email(user["email"], subject, html_body)
            logger.info(
                f"Successfully sent formatted RideCast to {user['name']}")

        except json.JSONDecodeError as e:
            logger.warning(
                f"Failed to parse JSON response from evaluator: {e}")
            ...
            html_body = create_fallback_email_html(
                chat_evaluation, user_first_name)

            send_email(user["email"], subject, html_body)
            logger.info(
                f"Successfully sent fallback RideCast to {user['name']}")

        except Exception as e:
            logger.error(
                f"Unexpected error processing email for {user['name']}: {e}")
            continue
